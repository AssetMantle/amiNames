...
const SocialModal = ({ isOpen, setIsOpen, type, profile, socialData }: any) => {
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const result = validateSocials(updatedSocials);
    const validatedSocialsLocal = { ...result?.validatedSocials };
    console.log("result: ", result);
    if (result?.error?.length > 0) {
      alert(`Invalid values for ${result?.error?.join?.(",")}`);
    } else {
      await addSocialData(validatedSocialsLocal, profile);
    }
    handleClose();
  };
